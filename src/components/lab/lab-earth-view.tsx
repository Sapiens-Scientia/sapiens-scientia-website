/* eslint-disable */
// LabEarthView: a duplicate of the site's UnifiedEarthView tuned for the
// You Are Here lab finale. Differences from the original: the camera can aim
// at the reader's home location on mount (cameraFocusOnHome), the home
...
function LocalRotationPath({ coords, isDark, theme, onProject }: { coords: EarthCoords; isDark: boolean; theme: ThemeMode; onProject?: (x: number, y: number, visible: boolean) => void }) {
    ...
    const homeMarkerRef = useRef<THREE.Mesh>(null)
    const projVec = useRef(new THREE.Vector3())

    // Report the home marker's on-canvas position each frame so the lab's
    // camera porthole can hang from it, hiding when the marker swings behind
    // the globe.
    useFrame(({ camera, gl }) => {
        if (!onProject || !homeMarkerRef.current) return
        const v = projVec.current
        homeMarkerRef.current.getWorldPosition(v)
        const facing = v.dot(camera.position) > 1.05
        v.project(camera)
        onProject(
            (v.x * 0.5 + 0.5) * gl.domElement.clientWidth,
            (-v.y * 0.5 + 0.5) * gl.domElement.clientHeight,
            facing && v.z < 1,
        )
    })
    ...
    return (
        <group>
            ...
            <mesh ref={homeMarkerRef} position={homePoint}>
...
function GalaxyHistoryModel({
    ...
    const galaxyDiskPosition = useMemo(
        () => new THREE.Vector3(0, presentPoint.y, 0),
        [presentPoint.y],
    )
    return (
        <group>
...
// ---------------------------------------------------------------------------
// A small caption for the planet itself, riding the globe's lower limb as
// seen from wherever the camera is — "Earth" always sits beneath its
// subject, the way the Moon's own label does.
// ---------------------------------------------------------------------------

function EarthLabel({ isDark }: { isDark: boolean }) {
    const groupRef = useRef<THREE.Group>(null)
    const work = useRef({ v: new THREE.Vector3(), r: new THREE.Vector3(), d: new THREE.Vector3() })
    useFrame(({ camera }) => {
        const { v, r, d } = work.current
        v.copy(camera.position).multiplyScalar(-1).normalize() // view direction at the globe
        r.crossVectors(v, ECLIPTIC_NORTH)
        if (r.lengthSq() < 1e-6) r.set(1, 0, 0)
        r.normalize()
        d.crossVectors(v, r).normalize() // screen-down, in world space
        groupRef.current?.position.copy(d.multiplyScalar(1.28))
    })
    return (
        <group ref={groupRef}>
            <Billboard>
                <Text fontSize={0.05} color={isDark ? '#c7cedd' : '#475569'} anchorX="center" anchorY="middle" outlineWidth={0.004} outlineColor={isDark ? '#020617' : '#ffffff'}>
                    Earth
                </Text>
            </Billboard>
        </group>
...
interface LabEarthViewProps {
    ...
    paused?: boolean
    /** When false, drag-to-rotate stays live but the wheel is left to page scroll. */
    enableWheelZoom?: boolean
